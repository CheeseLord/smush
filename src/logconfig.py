...
# Ensure the root logger has a handler, and set the root logger to log all
# messages.  This means that any logger that we don't explicitly call
# setLevel() on will log all messages, allowing the handlers to control which
# logs actually go through.
...
def newLogger(fullname):
    ...
    if len(name) > MAX_NAME_LENGTH:
        # Don't just use endLen for both beginning and end, because that's
        # wrong if MAX_NAME_LENGTH - len(middle) is odd. Round down for endLen
        # so that if we decide to set MAX_NAME_LENGTH to an even number, then
